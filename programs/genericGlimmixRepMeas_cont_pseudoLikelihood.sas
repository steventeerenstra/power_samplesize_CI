* to do: with proc mixed instead of proc glimmix;
...
**** Scenario 1:  sd=1.33 corresponderend met een IQR van 2-12 ***;
...
n_sim=1000, sim_block=100,seedstartrow=1, dir='.', store=%str(work), ds_out=dir.test1);
...
****** Scenario 2:  sd=0.96 corresponderend met een IQR van 3-11 ***;
%macro clussize_ClusterTimeTrt(); 
clussize=15;
%mend;
%let sd=0.963; 
...
%sim_cluster_repmeas(n_clus=8, n_rep=17, delta_trt=&delta_trt, data_time_effects=data_time_effects,
sigma2_c=&sigma2_c, proc_options=%str(method=rspl),
n_sim=1000, sim_block=100,seedstartrow=1, dir='.', store=%str(work), ds_out=dir.test2);
